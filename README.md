...
> **Independent web project** · Пет-проект, сделанный как полноценный продукт.
...
- `player.js` — плеер и очередь. `yt.js` — YouTube-резолвер (с декретно-фолбэк на CORS).
...
## Что сделал AI, а что — человек (честно)
Архитектура (SPA без зависимостей, разделение data/app/player), UX-решения (горячие клавиши, история слушаний), отладка данных и финальная проверка — **делались и перепроверялись вручную**. ИИ (DeepSeek + GLM) — ассистент генерации кода и каталога под человеческим контролем.
